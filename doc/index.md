...
**Obsidian API** is a library that allow you to implement an API between a web
...
* An application-side Class (inside the `iframe`) that will be used to
  ...
  integration,...),
* and a integration-side script (outside the `iframe`) that will be used as
...
* Allows the integration to send some configuration to the application before
  it was loaded,
* Allows to transfer of various types of objects (including `TypedArrays`,
  `Blob` and `Buffer`),
